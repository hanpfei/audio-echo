...
The src.audio is expected to glitch when the workload is high because there is too much work
and the src.audio task misses its delivery deadlines.
...
Measure touch screen latency plus src.audio output latency.
...
* Tap RECORD to record several seconds of src.audio. You should see the red VU meters move.
...
This test works with either a [loopback adapter](https://source.android.com/devices/src.audio/latency/loopback) or through speakers.
...
If #XRuns increments when a glitch occurs then the glitch is probably due to preemption of the src.audio task.
